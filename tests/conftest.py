...
from app.db.phase49_role_hierarchy_schema_sync import (
    ensure_phase49_role_hierarchy_columns,
    ensure_phase49_role_hierarchy_seed,
)
...
def test_engine():
    ...
    ensure_phase32_finance_quarterly_fee_bands_foundation(engine)
    ...
    ensure_phase41_performance_review_experience_foundation(engine)
    ensure_phase42_performance_skill_matrix_foundation(engine)
    ensure_phase43_quote_invoiced_date_foundation(engine)
    ensure_phase44_quote_invoicing_foundation(engine)
    ensure_phase45_performance_review_engine_foundation(engine)
    ensure_phase46_org_department_foundation(engine)
    ensure_phase47_user_lifecycle_foundation(engine)
    ...
    ensure_phase64_finance_planning_scenarios_foundation(engine)
    ensure_phase65_hr_form_publish_foundation(engine)
    ...
    ensure_phase77_command_center_foundation(engine)
    ensure_phase78_employee_immutable_foundation(engine)
    ensure_phase79_timesheet_home_team_foundation(engine)
    ...
    ensure_phase87_it_software_foundation(engine)
    ensure_phase88_it_master_data_foundation(engine)
    ensure_phase89_resource_shifts_foundation(engine)
    ensure_phase90_it_parent_asset_foundation(engine)
...
def seeded_db(test_session_factory, test_engine):
    ...
    ensure_phase32_finance_quarterly_fee_bands_foundation(test_engine)
    ...
    ensure_phase41_performance_review_experience_foundation(test_engine)
    ensure_phase42_performance_skill_matrix_foundation(test_engine)
    ensure_phase43_quote_invoiced_date_foundation(test_engine)
    ensure_phase44_quote_invoicing_foundation(test_engine)
    ensure_phase45_performance_review_engine_foundation(test_engine)
    ensure_phase46_org_department_foundation(test_engine)
    ensure_phase47_user_lifecycle_foundation(test_engine)
    ensure_phase48_compensation_change_foundation(test_engine)
    ensure_phase49_role_hierarchy_seed(test_engine)
    ensure_phase51_department_standardization(test_engine)
    ensure_phase52_ticketing_foundation(test_engine)
    ensure_phase53_ticket_routing_foundation(test_engine)
    ...
    ensure_phase77_command_center_foundation(test_engine)
    ensure_phase78_employee_immutable_foundation(test_engine)
    ensure_phase79_timesheet_home_team_foundation(test_engine)
    ...
    ensure_phase87_it_software_foundation(test_engine)
    ensure_phase88_it_master_data_foundation(test_engine)
    ensure_phase89_resource_shifts_foundation(test_engine)
    ensure_phase90_it_parent_asset_foundation(test_engine)
