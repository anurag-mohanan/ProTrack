...
# Executive tier (MD + Directors): company-wide oversight — read-all + approvals,
# but NOT operational write (that stays with FULL_ACCESS_ROLES / project specials).
EXECUTIVE_OVERSIGHT_ROLES = frozenset(EXECUTIVE_ROLES)
# Office Admin / HR monitor completion across all teams (view only — no approve).
# Executives also get all-teams timesheet visibility for company oversight.
TIMESHEET_COMPLIANCE_VIEWER_ROLES = frozenset({HR, OFFICE_ADMINISTRATOR}) | EXECUTIVE_OVERSIGHT_ROLES
READ_ALL_PROJECT_ROLES = (
    FULL_ACCESS_ROLES | {DESIGN_LEADER, READ_ONLY, PLANNING_BOARD} | EXECUTIVE_OVERSIGHT_ROLES
)
REPORT_VIEWER_ROLES = (
    FULL_ACCESS_ROLES | {DESIGN_LEADER, READ_ONLY, PLANNING_BOARD} | EXECUTIVE_OVERSIGHT_ROLES
)
RESOURCE_PLANNING_ROLES = FULL_ACCESS_ROLES | {PLANNING_BOARD} | EXECUTIVE_OVERSIGHT_ROLES
WORKLOAD_VIEWER_ROLES = FULL_ACCESS_ROLES | {DESIGN_LEADER, PLANNING_BOARD} | EXECUTIVE_OVERSIGHT_ROLES
